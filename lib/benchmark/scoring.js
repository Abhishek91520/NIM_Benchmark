...
export function computeLeaderboard(results = [], modelIds = [], weights = DEFAULT_WEIGHTS) {
  ...
  const modelsWithAverages = Object.values(modelStats).map((stat) => {
    ...
    const isWorking = stat.successfulCount > 0;
    const lastResult = stat.results[stat.results.length - 1];
    const errorReason = !isWorking
      ? (lastResult?.judgeReasoning || lastResult?.errorCode || "Request failed")
      : null;
    const sampleResponse = isWorking
      ? (stat.results.find((r) => r.success)?.response || "")
      : "";
    ...
    return {
      ...
      isWorking,
      status: isWorking
        ? stat.successfulCount === stat.totalCount
          ? "operational"
          : "degraded"
        : "failed",
      errorReason,
